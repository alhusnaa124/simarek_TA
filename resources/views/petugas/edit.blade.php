...
@section('content')
    <div class="container mx-auto p-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <form id="form-edit-petugas" method="POST" action="{{ route('petugas.update', $petugas->id) }}" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" id="alamat" name="alamat" value="{{ old('alamat', $petugas->alamat) }}"
                        class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-green-500">
                    @error('alamat')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="label_petugas" id="label_petugas"
                        class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-green-500">
                        <option value="">- Pilih Label Petugas -</option>

                        <optgroup label="Belum Digunakan">
                            @foreach ($allLabels as $label)
                                @if (!in_array($label, $usedLabels))
                                    <option value="{{ $label }}" @selected($petugas->label_petugas === $label)>
                                        {{ $label }}
                                    </option>
                                @endif
                            @endforeach
...
@push('scripts')
    ...
    <script>
        ...
        document.getElementById('form-edit-petugas').addEventListener('submit', function(e) {
            ...
            if (usedLabels.includes(selectedLabel)) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        document.getElementById('form-edit-petugas').submit();
                    }
                });
            }
        });
