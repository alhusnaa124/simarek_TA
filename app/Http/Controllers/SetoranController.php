...
class SetoranController extends Controller
{
    ...
    {
        $user = Auth::user();
        $status = $request->query('status', 'belum'); // default: "belum"
        $query = SetoranPBB::query();

        // Ambil setoran sesuai role
        if ($user->role === 'Petugas') {
            $query = SetoranPBB::whereHas('petugas', function ($query) use ($user) {
                $query->where('id_petugas', $user->id);
            });
        }

        // Filter berdasarkan status (sudah diverifikasi / belum)
        if ($status === 'sudah') {
        ...
        return view('setoran.setoran', compact('setoran'));
    }
    ...
    {
        // Validasi input, harus ada array formulir_ids yang berisi ID formulir
        $request->validate([
        ...
        // Ambil formulir yang dipilih dan belum ada setoran
        ...
        try {
            ...
            // Update kolom id_setoran di formulir yang dipilih dengan ID setoran baru
